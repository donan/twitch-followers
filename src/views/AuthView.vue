            ...
            <template v-if="!checking">
                <template v-if="!auth">
                    ...
                    <template v-if="invalid">
                        ...
                        <p><a @click.prevent="warningClearData" class="btn btn-danger">Clear browser Authorization data</a></p>
                        </template>
                </template>
                <template v-else>
                    <template>
                    ...
                    <p><em>If you have any auth error while displaying the followers list or entering this page, press this button:</em></p>
                    <p><a @click.prevent="warningClearData" class="btn btn-danger">Clear browser Authorization data</a></p>
                </template>
            </template>
            <template v-else>
...
export default {
    ...
    computed:{
        authURL(){
            return `${process.env.VUE_APP_AUTH_SERVER}${this.twitchAuthURL}?response_type=token&client_id=${process.env.VUE_APP_CID}&redirect_uri=${process.env.VUE_APP_SERVER_URL}auth`
        },
        currentAuth(){
    ...
    watch:{
        auth(newAuth) {
            if(newAuth){
                localStorage.twFollowsAuth = newAuth
            }
        }
    },
    methods:{
        ...
        clearAuth(){
            ...
            this.timeLeft=null
            if(this.currentAuth){
                localStorage.removeItem("twFollowsAuth")
            }
            if(this.currentLogin){
                localStorage.removeItem("twFollowsLogin")
            }
        },
        async validateAuth(latestAuth){
            ...
            try {
                ...
                localStorage.twFollowsLogin = resp_data.login
